...
% convert to single if mem is a problem
grid.yc=single(grid.yc);
grid.xc=single(grid.xc);
grid.zc=single(grid.zc);
...
config.prefilter.filterLow=single(config.prefilter.filterLow);
config.prefilter.filterHigh=single(config.prefilter.filterHigh);
...
if config.RieszOrder>0, % enable Riesz transform
    ...
    % ydc
    permutations = permn(0:config.RieszOrder, 3);
    validPerms = permutations(sum(permutations,2) == config.RieszOrder, :);
    config.RieszFilters = cell(1, config.RieszChannels);
    config.RieszOrders = cell(1, config.RieszChannels);
    for iter = 1:size(validPerms, 1)
        n = validPerms(iter, :);
        config.RieszOrders{iter} = n;
        config.RieszFilters{iter} = sqrt(multinomial(config.RieszOrder,n(1),n(2),n(3)))*base1.^(n(1)).*base2.^(n(2)).*base3.^(n(3));
    end
