function orientations = monogenicAnalysisSparseNoParFor(Q, config, J, sigma, idxMaskStruct, percentToKeep)
...
for j = J,
    ...
    for i=1:length(config.RieszOrders)
        % ydc: Only pure components fororder > 1
        band = find(config.RieszOrders{i} == config.RieszOrder);
        % band = find(config.RieszOrders{i});
        % end ydc: Not working for order > 1
        bandsIdx(band) = i;
    end
...
%     threshold = max(tmp(:))/2;
    ...
    % ydc 2015/02/26 - Optimization! (And bug correction).
    %[orientation1,orientation2,orientation3] = parallelOrientationsNoParFor(J11,J22,J33,J12,J13,J23,idxMask,w,h,d);
    [orientation1,orientation2,orientation3] = parallelOrientationsNoParForOptimized(J11,J22,J33,J12,J13,J23,idxMask);
    % ydc 2015/02/26 - Optimization! (And bug correction).
